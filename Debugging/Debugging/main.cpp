...
// commented the above line out, as its not needed in an empty project...
#include <iostream>
#include <string>
...
int main()
{
	...
	//removed int foo as it is not used anywhere in the code...
	//int foo = 0;
	int vowels = 0;
	int consonants = 0;
	int digits = 0;
	...
	int checkSum = 0;
	//removed int bar as it is not used anywhere in the code... also... haha foobar...
	//int bar = 0;
	...
	for (int i = 0; i < textToAnalyze.length(); ++i)
	{
		if (textToAnalyze[i] == 'a' || textToAnalyze[i] == 'e' || textToAnalyze[i] == 'i' ||
			...
			textToAnalyze[i] == 'U')
		{
			// Changed --vowels to ++vowels, -- decrements by one each revolution, 
			// We want to add 1 so we utilize ++
			// --vowels; to ++vowels;
			++vowels;
		}
		else if ((textToAnalyze[i] >= 'a'&& textToAnalyze[i] <= 'z') || (textToAnalyze[i] >= 'A'&& textToAnalyze[i] <= 'Z'))
		{
			//This code was commeneted out, and needed to be uncommented so we can add 1 to our consonants..
